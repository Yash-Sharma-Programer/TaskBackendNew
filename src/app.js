...
export const createApp = (io = null) => {
  ...
  // app.use(cors({ origin: (origin, callback) => !origin || env.clientUrls.includes(origin) ? callback(null, true) : callback(new Error('Origin not allowed by CORS')), credentials: true, methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'] }));
  const corsOptions = {
  ...
  // app.use(cors({
  //   origin:  'https://task-frontend-iota-ruby.vercel.app',
  //   credentials: true
  // }));
